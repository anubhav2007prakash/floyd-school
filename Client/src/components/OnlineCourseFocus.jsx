...
const OnlineCourseFocus = ({ variant = 'dark' }) => {
    ...
    const scrollToPartnerForm = () => {
        ...
        if (formEl) {
            ...
            navigate('/school-partnerships#partner-form');
        }
    };
    ...
    return (
        <section id="online-focus" className="relative overflow-hidden bg-[#060818] py-24 lg:py-32 border-t border-white/5">
            {/* Background Mesh */}
            <div
                className="absolute inset-0 pointer-events-none opacity-20"
                style={{ backgroundImage: 'radial-gradient(#94a3b8 1px, transparent 1px)', backgroundSize: '32px 32px' }}
            />
            <div className="absolute left-0 top-0 h-[600px] w-[600px] -ml-[300px] -mt-[300px] rounded-full bg-indigo-600/10 blur-[130px] pointer-events-none" />
            <div className="absolute bottom-0 right-0 h-[600px] w-[600px] -mr-[300px] -mb-[300px] rounded-full bg-purple-600/10 blur-[130px] pointer-events-none" />
            ...
            <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === 'customized' && (
                        ...
                        >
                            <div className="group relative overflow-hidden rounded-[32px] border border-amber-500/30 bg-gradient-to-b from-[#24140b] via-[#140b06] to-[#070508] p-8 sm:p-12 shadow-[0_30px_90px_rgba(2,6,23,0.8)]">
                                ...
                                <div className="flex flex-col justify-between h-full">
                                    ...
                                    <div className="flex items-center justify-between pb-8 border-b border-white/10 text-xs font-black uppercase tracking-[0.22em] text-slate-400">
                                        <span className="opacity-90">FLOYD SCHOOL</span>
                                        <span className="rounded-full bg-amber-500/15 px-3.5 py-1 text-[11px] font-black text-amber-400 border border-amber-500/30">
                                            {CUSTOMIZED_PROGRAM.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="py-8 grid lg:grid-cols-12 gap-8 items-center">
                                        <div className="lg:col-span-7">
                                            <div className="mb-4 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-amber-400">
                                                <span className="inline-block h-2 w-2 rounded-full bg-amber-400 animate-pulse" />
                                                {CUSTOMIZED_PROGRAM.badge}
                                            </div>

                                            <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black uppercase leading-[1.08] tracking-tight text-white whitespace-pre-line mb-4">
                                                {CUSTOMIZED_PROGRAM.title}
                                            </h3>

                                            <p className="text-sm sm:text-base text-slate-300 leading-relaxed font-medium mb-8 max-w-xl">
                                                {CUSTOMIZED_PROGRAM.description}
                                            </p>

                                            {/* Information Pills */}
                                            <div className="flex flex-wrap gap-2.5">
                                                {CUSTOMIZED_PROGRAM.pills.map((pill, pIdx) => (
                                                    <span
                                                        key={pIdx}
                                                        className="px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider bg-amber-500/15 border border-amber-500/30 text-amber-200"
                                                    >
                                                        {pill}
                                                    </span>
                                                ))}
                                        ...
                                        <div className="lg:col-span-5">
                                            <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-slate-900 shadow-2xl h-[260px] sm:h-[300px]">
                                                <img
                                                    src={CUSTOMIZED_PROGRAM.image}
                                                    alt={CUSTOMIZED_PROGRAM.imageAlt}
                                                    className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                                                />
                                                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent" />
                                                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-[10px] font-bold text-white/90 uppercase tracking-widest bg-black/60 backdrop-blur-md px-3.5 py-1.5 rounded-lg border border-white/10">
                                                    <span>{CUSTOMIZED_PROGRAM.category}</span>
                                                    <span className="text-amber-400">Tailored Fit</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                                        <button
                                            ...
                                            className="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-amber-500 via-orange-600 to-rose-600 px-8 py-4 text-xs sm:text-sm font-black uppercase tracking-widest text-white shadow-xl shadow-orange-600/30 transition-all hover:scale-[1.02] cursor-pointer"
                                        >
                                            DISCUSS YOUR SCHOOL
                                            <ArrowRight className="ml-2 h-4 w-4" />
                                        </button>
                                        ...
                                        <button
                                            ...
                                            className="inline-flex items-center justify-center gap-2 text-xs sm:text-sm font-bold text-slate-300 hover:text-white px-4 py-3 rounded-xl hover:bg-white/5 transition-colors cursor-pointer"
                                        >
                                            {showCustomDetails ? 'HIDE DETAILS ↑' : 'EXPLORE HOW IT WORKS →'}
                                        </button>
                                    </div>
                                </div>
                            </div>

                            {/* ── Customized Capabilities Explanation ── */}
                            <motion.div
                                initial={{ opacity: 0, height: 0 }}
                                animate={{ opacity: 1, height: 'auto' }}
                                className="rounded-3xl border border-amber-500/20 bg-gradient-to-b from-[#180d07] to-[#090508] p-8 sm:p-12 shadow-2xl"
                            >
                                <div className="text-center max-w-3xl mx-auto mb-10">
                                    <span className="text-[11px] font-black uppercase tracking-[0.25em] text-amber-400 block mb-2">Tailored Adaptability</span>
                                    <h3 className="text-2xl sm:text-4xl font-black uppercase text-white tracking-tight mb-3">
                                        YOUR SCHOOL. YOUR REQUIREMENTS. YOUR PROGRAM.
                                    </h3>
                                    <p className="text-slate-300 text-sm sm:text-base font-medium">
                                        Floyd can customize the learning experience around your exact institutional parameters.
                                    </p>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                                    {CUSTOM_CAPABILITIES.map((cap, cIdx) => {
                                        const Icon = cap.icon;
                                        return (
                                            <div key={cIdx} className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] flex flex-col justify-between">
                                                <div>
                                                    <div className="w-10 h-10 rounded-xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center mb-4 text-amber-400">
                                                        <Icon size={20} />
                                                    </div>
                                                    <h4 className="font-black text-white text-base uppercase tracking-tight mb-2">{cap.title}</h4>
                                ...
                                <div className="text-center pt-6 border-t border-white/10">
                                    <button
                                        type="button"
                                        onClick={scrollToPartnerForm}
                                        className="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-amber-500 to-rose-600 px-8 py-4 text-xs font-black uppercase tracking-widest text-white shadow-xl shadow-orange-600/30 transition-all hover:scale-[1.02] cursor-pointer"
                                    >
                                        BUILD YOUR PROGRAM WITH FLOYD
                                        <ArrowRight className="ml-2 h-4 w-4" />
                                    </button>
                                </div>
                            </motion.div>
